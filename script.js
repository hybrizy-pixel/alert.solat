...
function updateClock() {
    ...
    const malaysiaTime = now.toLocaleTimeString("en-GB", { hour12: false });
...
// =========================
// ENABLE NOTIFICATION (FIXED FOR V16)
// =========================

async function enableNotification() {
    try {
        if (!window.OneSignal) {
            alert("OneSignal belum sedia. Sila tunggu sekejap atau refresh page.");
            return;
        }

        console.log("Meminta kebenaran push notification...");
        
        // 1. Minta permission daripada browser
        await OneSignal.Notifications.requestPermission();
        
        // 2. Paksa status user untuk Opt-In masuk ke sistem database OneSignal
        await OneSignal.User.PushSubscription.optIn();
        
        // 3. Ambil ID subscription cara baharu SDK v16
        ...
        if (subscriptionId && isOptedIn) {
            ...
            alert("❌ Subscription Gagal. Sila pastikan anda memilih 'Allow' pada browser.");
        }

    } catch (error) {
        console.error("Gagal hidupkan notification:", error);
...
async function testNotification() {
    try {
        ...
        alert("✅ Push Sent. Sila semak peranti anda.");
    } catch (error) {
        console.log(error);
        alert("Gagal menghantar test alert.");
    }
}
